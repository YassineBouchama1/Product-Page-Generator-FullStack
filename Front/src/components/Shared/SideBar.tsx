...
export default function SideBar() {
  ...
  const logOutBtn = async () => {
    const token = await Cookies.remove('token')
    const user = await Cookies.remove('user')
    const isLOgin = await Cookies.remove('loggedin')
    const all = await Cookies.remove()
      ...
      <div className="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800  ">
        <ul className="space-y-2 font-medium">
          ...
          <li className="mt-20">
            <button
              onClick={() => logOutBtn()}
              className={`none flex items-center p-2 text-gray-900 rounded-lg
               dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700`}
            >
              <svg
                className="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75  dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
